...
namespace BTL_2023_c
{
    public partial class QLMH : Form
    {
        ...
        void Decentralization()
        {
            ...
            {
                btn_Them.Enabled = btn_Sua.Enabled = btn_Xoa.Enabled = cb_khoitao.Enabled = false;
            }
        }
        ...
        public void laydataGridMH()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT sMaMH AS 'Mã môn học' , sTenMH AS 'Tên môn học' , iSoTC AS 'Số tín chỉ' FROM tblMonHoc", cnn))
                {
        ...
        public static bool themMH(string maMH, string tenMH, int soTC)
        {
            ...
            using (SqlConnection cnn = new SqlConnection(counter))
            {
                using (SqlCommand cmd = cnn.CreateCommand())
                {
                    ...
                    cmd.Parameters.AddWithValue("@tenmon", tenMH);
                    cmd.Parameters.AddWithValue("@sotin", soTC);
                    cnn.Open();
        ...
        public static bool xoaMH(string maMH,string tenMH, int soTC)
        {
            string counter = ConfigurationManager.ConnectionStrings["btl_C#"].ConnectionString;
            using (SqlConnection cnn = new SqlConnection(counter))
            {
        ...
        {
            xoaMH(Convert.ToString(mask_maMH.Text),Convert.ToString(mask_tenMH.Text),int.Parse(mask_soTC.Text));
            laydataGridMH();
        }
        ...
        private void cb_khoitao_Click(object sender, EventArgs e)
        {
            mask_maMH.Text = "";
            mask_tenMH.Text = "";
            mask_soTC.Text = "";
