...
namespace BTL_2023_c
{
    public partial class QLKhoa : Form
    {
        ...
        void Decentralization()
        {
            ...
            {
                btn_themKhoa.Enabled = btn_suaKhoa.Enabled = btn_xoaKhoa.Enabled = bt_khoitao.Enabled = false;
            }
        }
        public void laydataGridKhoa()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT sMaKhoa AS 'Mã khoa' , sTenKhoa AS 'Tên khoa' , sSDT AS 'Số điện thoại' , sDiaChiKhoa AS 'Địa chỉ khoa' FROM tblKhoa", cnn))
                {
        ...
        private void dtGridKhoa_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            mask_sdt.Text = dtGridKhoa.CurrentRow.Cells["Số điện thoại"].Value.ToString();
            mask_diaChi.Text = dtGridKhoa.CurrentRow.Cells["Địa chỉ khoa"].Value.ToString();
        }
