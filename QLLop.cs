...
namespace BTL_2023_c
{
    public partial class QLLop : Form
    {
        ...
        void Decentralization()
        {
            ...
            {
                btn_ThemLop.Enabled = btn_suaLop.Enabled = btn_xoaLop.Enabled = bt_khoitao.Enabled = false;
            }
        }
        ...
        public void laydataGridLop()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT sMaLop AS 'Mã lớp' , sTenLop AS 'Tên lớp' , sMaKhoa AS 'Mã khoa' FROM vw_dtgridLop", cnn))
                {
                    cmd.CommandType = CommandType.Text;
                    using (SqlDataAdapter ad = new SqlDataAdapter(cmd))
                    {
